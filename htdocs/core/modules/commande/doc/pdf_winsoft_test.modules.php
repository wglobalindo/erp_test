...
/**
 *	\file       htdocs/core/modules/commande/doc/pdf_winsoft_test.modules.php
 *	\ingroup    commande
 *	\brief      Fichier de la classe permettant de generer les commandes au modele winsoft test
 */
...
/**
 *	Classe to generate PDF orders with template winsoft test
 */
class pdf_winsoft_test extends ModelePDFCommandes
{
    /**
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "winsoft_test";
		$this->description = $langs->trans('winsoft test');
		...
		if ($conf->commande->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				for ($i = 0 ; $i < $nblignes ; $i++)
				{
					...
					if (empty($object->lines[$i]->array_options['options_cn'])){
						...
						$pdf->SetXY($curX, $curY);
						$pdf->MultiCell ($this->posxtva-$curX,3, $outputlangs->convToOutputCharset($object->lines[$i]->array_options['options_cn']),0,'L');
					}
					...
					if ($currency_bill=="IDR")
            {
				...
				if ($pagenb == 1)
					...
					$this->_tableau($pdf, $tab_top_newpage-7, $this->page_hauteur - $tab_top_newpage - $heightforinfotot - $heightforfreetext - $heightforfooter, 0, $outputlangs, 0, 0, $object->multicurrency_code);
    ...
    foreach($this->tva as $tvakey => $tvaval)
    {
      if ($tvakey != 0)    // On affiche pas taux 0
      {
        ...
        $total_ht = ($conf->multicurrency->enabled && $object->mylticurrency_tx != 1 ? $object->multicurrency_total_ht : $object->total_ht);
    		...
    		$pdf->MultiCell($largcol2, $tab2_hl,$currency_bill.' '.price($total_ht + (! empty($object->remise)?$object->remise:0), 0, $outputlangs), 0, 'R', 1);
      }
    }
		//$pdf->MultiCell($largcol2, $tab2_hl,$currency_bill.' '.price($total_ht + (! empty($object->remise)?$object->remise:0), 0, $outputlangs), 0, 'R', 1);
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $titlekey="PdfOrderTitle")
	{
		...
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(100, 4, $outputlangs->transnoentities("CustomerCode")." : " . $outputlangs->convToOutputCharset($object->ref), '', 'R');
		...
		if ($showaddress)
		{
			...
			//$carac_client=dol_format_address($object,1,$sep = "\n",$outputlangs ,$mode = 0);
     ...
     *
	 *   	@param	TCPDF		$pdf     			PDF
	 * 		@param	Object		$object				Object to showż
	 *      @param	Translate	$outputlangs		Object lang for output
