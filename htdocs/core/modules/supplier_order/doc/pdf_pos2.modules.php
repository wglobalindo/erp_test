...
/**
 *	\file       htdocs/core/modules/supplier_order/doc/pdf_POS2.modules.php
 *	\ingroup    fournisseur
 *	\brief      File of class to generate suppliers orders from POS2 model
 */
...
/**
 *	Class to generate the supplier orders with the POS2 model
 */
class pdf_POS2 extends ModelePDFSuppliersOrders
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "POS2";
		$this->description = $langs->trans('SuppliersCommandModelPOS2');

		// Page size for A4 format
		$this->type = 'pdf';
		$formatarray = pdf_getFormat();
		//$this->page_largeur = $formatarray['width'];
		$this->page_largeur = "58";
		//$this->page_hauteur = $formatarray['height'];
		//$this->page_hauteur = $formatarray['height'];
		$this->page_hauteur = "500";
	...
	public function write_file($object, $outputlangs = '', $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->fournisseur->commande->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				$pdf->SetFont('', '', 8);
				$pdf->MultiCell(0, 0, ''); // Set interline to 3
				...
				for ($i = 0; $i < $nblines; $i++)
				{
					...
					$curY = $nexY;
					$pdf->SetFont('', '', 8); // Into loop to work with multipage
					$pdf->SetTextColor(0, 0, 0);
					...
					pdf_writelinedesc($pdf, $object, $i, $outputlangs, 48, 0, $curX, $curY, $hideref, $hidedesc, 1);
					...
					if ($pageposafter > $pageposbefore)	// There is a pagebreak
					{
						...
						pdf_writelinedesc($pdf, $object, $i, $outputlangs, $descWidth, 0, $curX, $curY, $hideref, $hidedesc, 1);
						$posyafter = $pdf->GetY();
						if ($posyafter > $page_hauteur)	// There is no space left for total+free text
						{
					...
					$pdf->SetFont('', '', 8); // On repositionne la police par defaut
					...
					*/
					...
					if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_PURCHASE_ORDER_WITHOUT_TOTAL_COLUMN)) {
						$total_excl_tax = pdf_getlinetotalexcltax($object, $i, $outputlangs);
						$pdf->SetXY($this->posxdesc-1, $nexY);
						$pdf->MultiCell($this->page_largeur - $this->marge_droite - $this->postotalht, 3, "Rp ".$total_excl_tax, 0, 'R', 0);
					}
					...
					if ($i == ($nblines - 1))	// No more lines, and no space left to show total, so we create a new page
					{
						...
						$pdf->SetXY($this->posxdesc-1, $nexY+10);
				...
				if ($pagenb == 1)
				{
					...
					$pdf->SetFont('', '', 7);
					//Date
					$titre = dol_print_date($object->date_commande, "day", false, $outputlangs, true);
					$pdf->SetXY(0, 2 );
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		*/
		...
		if (!empty($object->date_commande))
		{
			...
			$pdf->MultiCell(100, 3, dol_print_date($object->date_commande, "day", false, $outputlangs, true), '', 'L');
		} else {
			$posy += 5;
			$pdf->SetXY($posx, $posy);
