...
/**
 *	\file       htdocs/core/modules/supplier_order/pdf/pdf_supplier_order.modules.php
 *	\ingroup    fournisseur
 *	\brief      File of class to generate suppliers orders from supplier_order model
 */
...
/**
 *	Class to generate the supplier orders with the supplier_order model
 */
class pdf_pos extends ModelePDFSuppliersOrders
{
    var $db;
    var $name;
    var $description;
	...
	{
		global $user,$langs,$conf,$hookmanager,$db,$mysoc,$nblignes;
		...
		if ($conf->fournisseur->commande->dir_output)
		{
			...
			if (file_exists($dir))
			{
                ...
                $default_font_size = 3;	// Must be after pdf_getInstance
                $heightforinfotot = 3;	// Height reserved to output the info and total part
				...
				for ($i = 0 ; $i < $nblignes ; $i++)
				{
					...
					*/
					...
					//$pdf->MultiCell(0, 0, $up_excl_tax." x ".$qty."> ".$nexY, 0, 'L', 0);
					$pdf->MultiCell($this->page_largeur-$this->marge_droite-$this->postotalht, 0, $total_excl_tax, 0, 'L', 0);
					...
					if ($object->lines[$i]->remise_percent)
					{
						...
						$pdf->MultiCell($this->postotalht-$this->posxdiscount-1, 0, $remise_percent, 0, 'R');
					}*/

					// Total HT line
                    $total_excl_tax = pdf_getlinetotalexcltax($object, $i, $outputlangs);
					$pdf->SetXY($this->postotalht, $nexY);
					$pdf->MultiCell($this->page_largeur-$this->marge_droite-$this->postotalht, 0, $total_excl_tax, 0, 'R', 0);
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0, $currency='')
	{
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell(0,2, $outputlangs->transnoentities("TotalHTShort"),'','C');
		}
	}
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		if ($this->emetteur->logo)
		{
			if (is_readable($logo))
			{
			    ...
			    $pdf->Image($logo, $this->marge_gauche, $posy, 5, 5);	// width=0 (auto)
			}
			else
			{
		...
		if ($object->array_options['options_cspo'])
		{
			$cspoid = new commande($this->db);
        	$cspoid->fetch($object->array_options['options_cspo']);
        	$cspo=$cspoid->ref;
			...
			$pdf->SetXY($posx,$posy);
			$pdf->SetTextColor(0,0,60);
			$pdf->MultiCell(100, 3, $outputlangs->transnoentities("Customer PO")." : " . $outputlangs->convToOutputCharset($cspo), '', 'R');
		}
