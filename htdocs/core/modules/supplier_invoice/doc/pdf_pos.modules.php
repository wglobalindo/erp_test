...
/**
 *	\file       htdocs/core/modules/supplier_invoice/doc/pdf_POS.modules.php
 *	\ingroup    fournisseur
 *	\brief      Class file to generate the supplier invoices with the POS model
 */
...
/**
 *	Class to generate the supplier invoices PDF with the template POS
 */
class pdf_POS extends ModelePDFSuppliersInvoices
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->page_largeur = "48";
		//$this->page_hauteur = $formatarray['height'];
		$this->page_hauteur = "300";
		$this->format = array($this->page_largeur, $this->page_hauteur);
		$this->marge_gauche = isset($conf->global->MAIN_PDF_MARGIN_LEFT) ? $conf->global->MAIN_PDF_MARGIN_LEFT : 0;
		$this->marge_droite = isset($conf->global->MAIN_PDF_MARGIN_RIGHT) ? $conf->global->MAIN_PDF_MARGIN_RIGHT : 0;
		$this->marge_haute = isset($conf->global->MAIN_PDF_MARGIN_TOP) ? $conf->global->MAIN_PDF_MARGIN_TOP : 0;
		...
		if ($conf->fournisseur->facture->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				for ($i = 0; $i < $nblines; $i++)
				{
					...
					pdf_writelinedesc($pdf, $object, $i, $outputlangs, $this->page_largeur - $curX, 3, $curX, $curY, $hideref, $hidedesc, 1);
					...
					if ($pageposafter > $pageposbefore)	// There is a pagebreak
					{
						...
						pdf_writelinedesc($pdf, $object, $i, $outputlangs, $this->page_largeur - $curX, 4, $curX, $curY, $hideref, $hidedesc, 1);
						$posyafter = $pdf->GetY();
					...
					$pdf->SetXY($this->posxdesc-1, $nexY);
					$pdf->MultiCell(0, 0, $up_excl_tax." x ".$qty, 0, 'L', 0);
					...
					if ($currency_bill=="IDR")
          			{
						...
						$total_excl_tax = "Rp ".number_format($total_excl_tax,2,',','.');
          			}
					$pdf->SetXY($this->posxdesc-1, $nexY);
					$pdf->MultiCell($this->page_largeur - $this->marge_droite - $this->postotalht, 3, $total_excl_tax, 0, 'R', 0);
					...
					*/
					...
					$nexY += 8; // Add space between lines

					// Customer PO Ref
					if($object->lines[$i]->array_options['options_cp']) {
						require_once DOL_DOCUMENT_ROOT.'/commande/class/commande.class.php';
						$pdf->SetXY($this->posxdesc-1, $pdf->GetY());

						$objectcp = new Commande($this->db);
						$objectcp->fetch($object->lines[$i]->array_options['options_cp']);
						$pdf->MultiCell(0, 0,  $objectcp->ref, 0, 'L', 0);
						$nexY += 2;
					}
					...
					if ($i == ($nblines - 1))	// No more lines, and no space left to show total, so we create a new page
					{
						...
						$pdf->SetXY($this->posxdesc-1, $nexY+5);
						$pdf->MultiCell(0, 0, "Total", 0, 'L', 0);
						$pdf->SetXY($this->posxdesc-1, $nexY+5);
						if ($currency_bill=="IDR")
          				{
							$total_ttc = sprintf("%.2f", filter_var(price($total_ttc),519)*0.01);
							$total_ttc = "Rp ".number_format($total_ttc,2,',','.');
          				}
						$pdf->MultiCell($this->page_largeur - $this->marge_droite - $this->postotalht, 0, $total_ttc, 0, 'R', 0);
						//$pdf->SetXY($this->posxdesc-1, $nexY+20);
						//$pdf->MultiCell(0, 0, "END", 0, 'L', 0);
						//$pdf->SetXY($this->posxdesc-1, $nexY+5);
						//$pdf->MultiCell($this->page_largeur - $this->marge_droite - $this->postotalht, 0, "Rp ".price($total_ttc), 0, 'R', 0);
					}
				}
				...
				if ($pagenb == 1)
				{
					...
					$pdf->SetFont('', '', 7);
					//Date
					$titre = dol_print_date($object->date, "day", false, $outputlangs, true);
					$pdf->SetXY(0, 2 );
					$pdf->MultiCell(($pdf->GetStringWidth($titre) + 3), 2, $titre);

					//Ref
					$pdf->SetXY(0, $pdf->GetY());
					$Ref = $outputlangs->convToOutputCharset($object->ref);
					$pdf->MultiCell('','', $Ref, '', 'L');

					//Seller
					$thirdparty = $object->thirdparty;
					$Seller = pdfBuildThirdpartyName($thirdparty, $outputlangs);
					$pdf->SetXY(0, 2);	
					$pdf->MultiCell('0','0', $Seller, '', 'R');

					//Customer
					//$Customer = pdfBuildThirdpartyName($thirdparty, $outputlangs);
					$Customer = "WG";
					$pdf->SetXY(0, $pdf->GetY());	
					$pdf->MultiCell('0','0', $Customer, '', 'R');

					$pdf->SetLineStyle(array('dash'=>'1,1') );
					$pdf->line($this->marge_gauche, 0, $this->page_largeur - $this->marge_droite, 0);
					$pdf->line($this->marge_gauche, 10, $this->page_largeur - $this->marge_droite, 10);

					//$bottomlasttab = $this->page_hauteur - $heightforinfotot - $heightforfreetext - $heightforfooter + 1;
				} else {

					//$this->_tableau($pdf, $tab_top_newpage, $this->page_hauteur - $tab_top_newpage - $heightforinfotot - $heightforfreetext - $heightforfooter, 0, $outputlangs, 1, 0, $object->multicurrency_code);
					//$bottomlasttab = $this->page_hauteur - $heightforinfotot - $heightforfreetext - $heightforfooter + 30;
